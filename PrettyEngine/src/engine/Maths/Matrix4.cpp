...
namespace PrettyEngine {
	...
	Matrix4 Matrix4::LookAt(const Vector3& camera, const Vector3& object, const Vector3& up)
	{
		...
		Vector3 f = (object - camera).Normalise();
		Vector3 s = f.CrossProduct(up.Normalise());
	...
	String Matrix4::ToString() const
	{
		...
		result << "Matrix4: (" << rows[0].x << ", " << rows[1].x << ", " << rows[2].x << ", " << rows[3].x << "), ";
	...
	String Matrix4::Export() const
	{
		std::stringstream result;
		result << rows[0].x << " " << rows[0].y << " " << rows[0].z << " " << rows[0].w << " ";
		result << rows[1].x << " " << rows[1].y << " " << rows[1].z << " " << rows[1].w << " ";
		result << rows[2].x << " " << rows[2].y << " " << rows[2].z << " " << rows[2].w << " ";
		result << rows[3].x << " " << rows[3].y << " " << rows[3].z << " " << rows[3].w << " ";
		return result.str();
	}
